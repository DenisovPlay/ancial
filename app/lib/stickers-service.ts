...
export function renderStickerHtml(code: string, imageUrl: string, isStandalone: boolean = false): string {
  ...
  return `<span class="inline-sticker-wrapper relative group inline-block cursor-pointer active:scale-95 duration-300" data-sticker="${safeCode}" contenteditable="false"><img class="${imgClass} copy" data-clipboard-text=":${safeCode}:" src="${safeUrl}" alt=":${safeCode}:" loading="lazy" /></span>`;
}
