    ...
    <NotificationContext.Provider value={{ showNote }}>
      ...
      <div className="fixed top-4 right-4 z-[9999] flex flex-col gap-2 pointer-events-none">
        {notes.map((note) => (
          <div
            key={note.id}
            className={`pointer-events-auto flex items-center justify-between p-4 min-w-[250px] max-w-sm shadow-xl rounded-2xl backdrop-blur-md border transition-all duration-300 animate-in fade-in slide-in-from-top-5 slide-in-from-right-5 ${
          ...
          >
            ...
            <button onClick={() => removeNote(note.id)} className="ml-4 p-1 opacity-60 hover:opacity-100 transition-opacity">
