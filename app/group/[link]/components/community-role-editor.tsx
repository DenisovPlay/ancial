...
export default function CommunityRoleEditor({ communityId, onChanged, onOpenView, roleList, view }: Props) {
  ...
  return <div className="flex flex-col gap-3">
    {view === 'overview' ? <><button type="button" onClick={openCreate} className={roleCreateAction}>{lang?.community_create_role}</button>{roleList.roles.map((role) => <div key={role.id} className="flex items-center gap-3 rounded-3xl border border-zinc-600/30 bg-zinc-800/60 p-3"><span className="h-3 w-3 rounded-full" style={{ backgroundColor: role.color }} /><span className="min-w-0 flex-1 truncate font-semibold">{role.name}</span><span className="text-xs text-zinc-500">{role.member_count}</span><button type="button" aria-label={lang?.edit || ''} title={lang?.edit || ''} onClick={() => openEdit(role)} className="flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-full bg-zinc-700 text-zinc-100 duration-300 hover:bg-zinc-600 active:scale-95"><svg className="h-5 w-5 fill-current" viewBox="0 0 48 48" aria-hidden="true"><use href="#IC-edit" /></svg></button>{!role.is_system ? <button type="button" aria-label={lang?.delete || ''} title={lang?.delete || ''} onClick={() => setPendingRoleId(role.id)} className="flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-full bg-red-600/20 text-red-300 duration-300 hover:bg-red-600 hover:text-white active:scale-95"><svg className="h-5 w-5 fill-current" viewBox="0 0 48 48" aria-hidden="true"><use href="#IC-trash" /></svg></button> : null}</div>)}</> : null}
