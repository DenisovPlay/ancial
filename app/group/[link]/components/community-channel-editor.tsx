...
export default function CommunityChannelEditor({ communityId, onChanged, onOpenView, roles, structure, view }: Props) {
  ...
  return <div className="flex flex-col gap-3">
    {view === 'overview' ? <><div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><button type="button" onClick={() => onOpenView('create_channel')} className={primary}>{lang?.community_create_channel}</button><button type="button" onClick={() => onOpenView('categories')} className={primary}>{lang?.community_create_category}</button><button type="button" onClick={() => onOpenView('channel_permissions')} className={primary}>{lang?.community_channel_permissions}</button></div>{structure.channels.map((channel) => <div key={channel.id} className="flex items-center gap-3 rounded-3xl border border-zinc-600/30 bg-zinc-800/60 p-3"><span className="min-w-0 flex-1 truncate font-semibold">{channel.title}</span>{channel.voice_enabled ? <span className="text-xs text-green-400">{lang?.community_channel_calls}</span> : null}<button type="button" onClick={() => openEdit(channel)} className="cursor-pointer rounded-3xl bg-zinc-700 px-3 py-1.5 duration-300 active:scale-95">{lang?.edit}</button><button type="button" onClick={() => setPendingDelete({ id: channel.id, kind: 'channel' })} className="cursor-pointer rounded-3xl bg-red-600/20 px-3 py-1.5 text-red-300 duration-300 active:scale-95">{lang?.delete}</button></div>)}</> : null}
