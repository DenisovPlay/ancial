...
export default function CommunityChannelEditor({ communityId, onChanged, onOpenView, roles, structure, view }: Props) {
  ...
  const back = (heading: string) => <div className="flex items-center gap-3"><button type="button" aria-label={lang?.back || ''} onClick={() => { resetChannel(); onOpenView('overview'); }} className="flex h-10 w-10 cursor-pointer items-center justify-center rounded-full bg-zinc-800 duration-300 active:scale-95"><Icon name="IC-chevron-left" className="size-5 fill-current" /></button><h3 className="text-lg font-semibold">{heading}</h3></div>;
  ...
  const overviewAction = `${primary} min-w-0 px-2 text-xs leading-tight sm:px-3 sm:text-sm`;
  const channelForm = (action: 'create' | 'update') => <div className="flex flex-col gap-3">{back(action === 'create' ? (lang?.community_create_channel || '') : (lang?.community_edit_channel || ''))}<input aria-label={lang?.community_channel_name} value={title} onChange={(event) => setTitle(event.target.value)} placeholder={lang?.community_channel_name} className={inputClass} /><select aria-label={lang?.community_categories} value={categoryId ?? ''} onChange={(event) => setCategoryId(event.target.value ? Number(event.target.value) : null)} className={inputClass}><option value="">{lang?.community_channel_uncategorized}</option>{structure.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><Toggle checked={voiceEnabled} label={lang?.community_channel_calls || ''} onChange={setVoiceEnabled} /><Toggle checked={readOnly} label={lang?.community_channel_read_only || ''} onChange={setReadOnly} /><label className="flex flex-col gap-1 text-sm text-zinc-300"><span>{lang?.community_slow_mode || 'Slow mode'}</span><input type="number" min={0} max={21600} value={slowMode} onChange={(event) => { const value = event.target.valueAsNumber; setSlowMode(Number.isFinite(value) ? value : 0); }} className={inputClass} /></label><button type="button" disabled={saving || !title.trim()} onClick={() => void saveChannel(action)} className={primary}>{action === 'create' ? lang?.community_create_channel : lang?.save}</button></div>;

  return <div className="flex flex-col gap-3">
    {view === 'overview' ? <><div className="grid grid-cols-3 gap-2 sm:gap-3"><button type="button" onClick={() => onOpenView('create_channel')} className={overviewAction}>{lang?.community_create_channel}</button><button type="button" onClick={() => onOpenView('categories')} className={overviewAction}>{lang?.community_create_category}</button><button type="button" onClick={() => onOpenView('channel_permissions')} className={overviewAction}>{lang?.community_channel_permissions}</button></div>{structure.channels.map((channel) => <div key={channel.id} className="flex items-center gap-2 rounded-3xl border border-zinc-600/30 bg-zinc-800/60 p-3 sm:gap-3"><div className="min-w-0 flex-1"><span className="block truncate font-semibold">{channel.title}</span>{channel.voice_enabled ? <span className="block truncate text-xs text-green-400">{lang?.community_channel_calls}</span> : null}</div><button type="button" aria-label={lang?.edit || ''} title={lang?.edit || ''} onClick={() => openEdit(channel)} className="flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-full bg-zinc-700 text-zinc-100 duration-300 hover:bg-zinc-600 active:scale-95"><Icon name="IC-edit" className="h-5 w-5 fill-current" aria-hidden="true" /></button><button type="button" aria-label={lang?.delete || ''} title={lang?.delete || ''} onClick={() => setPendingDelete({ id: channel.id, kind: 'channel' })} className="flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-full bg-red-600/20 text-red-300 duration-300 hover:bg-red-600 hover:text-white active:scale-95"><Icon name="IC-trash" className="h-5 w-5 fill-current" aria-hidden="true" /></button></div>)}</> : null}
    {view === 'create_channel' ? channelForm('create') : null}{view === 'edit_channel' ? channelForm('update') : null}
    {view === 'categories' ? <div className="flex flex-col gap-3">{back(lang?.community_categories || '')}<div className="flex gap-3"><input aria-label={lang?.community_category_name || ''} value={categoryName} onChange={(event) => setCategoryName(event.target.value)} placeholder={lang?.community_category_name} className={`min-w-0 flex-1 ${inputClass}`} /><button type="button" aria-label={lang?.community_create_category || ''} title={lang?.community_create_category || ''} disabled={!categoryName.trim()} onClick={() => void createCategory()} className={`${primary} flex h-12 w-12 shrink-0 items-center justify-center p-0 disabled:cursor-not-allowed`}><Icon name="IC-plus" className="h-6 w-6 fill-current" aria-hidden="true" /></button></div>{structure.categories.map((category) => <button key={category.id} onClick={() => setPendingDelete({ id: category.id, kind: 'category' })} className="cursor-pointer rounded-3xl bg-zinc-800 p-3 text-left duration-300 active:scale-95">{category.name} ×</button>)}</div> : null}
    {view === 'channel_permissions' ? <div className="flex flex-col gap-3">{back(lang?.community_channel_permissions || '')}<select aria-label={lang?.community_select_channel || ''} value={overrideDialogId || ''} onChange={(event) => { setOverrideDialogId(Number(event.target.value)); setOverrideValues({}); }} className={inputClass}><option value="">{lang?.community_select_channel}</option>{structure.channels.map((channel) => <option key={channel.id} value={channel.id}>{channel.title}</option>)}</select><select aria-label={lang?.community_select_role || ''} value={overrideRoleId || ''} onChange={(event) => { setOverrideRoleId(Number(event.target.value)); setOverrideValues({}); }} className={inputClass}><option value="">{lang?.community_select_role}</option>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select>{COMMUNITY_CHANNEL_PERMISSION_NAMES.map((permission) => <div key={permission} className="flex flex-col gap-2 rounded-3xl bg-zinc-800/60 p-3 sm:flex-row sm:items-center"><span className="min-w-0 flex-1 text-sm">{lang?.[`community_permission_${permission}`]}</span><div className="grid grid-cols-3 gap-1">{(['inherit', 'allow', 'deny'] as OverrideValue[]).map((value) => <button key={value} type="button" onClick={() => setOverrideValues((current) => ({ ...current, [permission]: value }))} className={`cursor-pointer rounded-3xl px-3 py-1.5 text-xs duration-300 active:scale-95 ${overrideValues[permission] === value || (!overrideValues[permission] && value === 'inherit') ? value === 'deny' ? 'bg-red-600' : value === 'allow' ? 'bg-green-600' : 'bg-purple-600' : 'bg-zinc-700'}`}>{lang?.[`community_${value}`]}</button>)}</div></div>)}<button onClick={() => void saveOverride()} disabled={!overrideDialogId || !overrideRoleId} className={primary}>{lang?.save}</button></div> : null}
