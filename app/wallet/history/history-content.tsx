...
export default function HistoryContent() {
  ...
  const getTransactionKind = (trans: WalletTransaction): 'internal' | 'in' | 'out' => {
    if (trans.is_internal) return 'internal';

    const senderOwned = ownedAccountIds.has(trans.sender);
    const receiverOwned = ownedAccountIds.has(trans.receiver);
  ...
  const getPartyLabel = (accountId: number, resolvedName?: string) => {
    if (accountId === -1) return strings.system;
    return resolvedName || String(accountId);
  };

  const getTransactionIcon = (trans: WalletTransaction) => {
    if (trans.status === 0) {
      return (
        <div className="border border-zinc-600/30 shrink-0 flex justify-center items-center bg-yellow-500/25 h-10 w-10 lg:h-12 lg:w-12 duration-300 rounded-full">
          <svg className="fill-yellow-500 h-6 w-6 lg:w-8 lg:h-8 inline" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
    ...
    if (trans.status === 2) {
      return (
        <div className="border border-zinc-600/30 shrink-0 flex justify-center items-center bg-red-500/25 h-10 w-10 lg:h-12 lg:w-12 duration-300 rounded-full">
          <svg className="fill-red-500 h-6 w-6 lg:w-8 lg:h-8 inline" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
    ...
    const kind = getTransactionKind(trans);

    if (kind === 'internal') {
      return (
        <div className="border border-zinc-600/30 shrink-0 flex justify-center items-center bg-blue-500/25 h-10 w-10 lg:h-12 lg:w-12 duration-300 rounded-full">
          <svg className="fill-blue-500 h-6 w-6 lg:w-8 lg:h-8 inline" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
    ...
    if (kind === 'in') {
      return (
        <div className="border border-zinc-600/30 shrink-0 flex justify-center items-center bg-green-500/25 h-10 w-10 lg:h-12 lg:w-12 duration-300 rounded-full">
          <svg className="fill-green-500 h-6 w-6 lg:w-8 lg:h-8 inline" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
  ...
  const getTransactionTextClass = (trans: WalletTransaction) => {
    if (trans.status === 2) return 'text-zinc-500';
    const kind = getTransactionKind(trans);
    if (kind === 'internal') return 'text-zinc-500';
    if (kind === 'in') return 'text-green-500';
    return 'text-red-500';
  };

  const getTransactionPrefix = (trans: WalletTransaction) => {
    if (trans.status === 2) return '';
    const kind = getTransactionKind(trans);
  ...
  const getTransactionDirectionName = (trans: WalletTransaction) => {
    const kind = getTransactionKind(trans);
    ...
    return `${trans.sender}->${getPartyLabel(trans.receiver, trans.receiver_name)}`;
  };
            ...
            <div className={`w-full transition-opacity duration-200 ${isRefreshing ? 'opacity-85' : 'opacity-100'}`}>
              {transactions.map((trans) => (
                <div
                  ...
                  className="flex gap-3 items-center w-full p-3 hover:bg-zinc-700/50 duration-300 cursor-pointer active:scale-95 active:rounded-3xl"
                >
            ...
            <div className="bg-zinc-800/60 rounded-3xl p-4 w-full text-sm space-y-2.5 border border-zinc-600/30">
              ...
              <div className="flex justify-between items-center border-b border-zinc-700 pb-2">
                ...
                <span className="font-semibold text-zinc-200">
                  {getTransactionKind(selectedTransaction) === 'internal'
                    ? 'Между счетами'
                    : getTransactionKind(selectedTransaction) === 'in'
                      ? 'Входящий'
                      : 'Исходящий'}
                </span>
              </div>
              <div className="flex justify-between items-center border-b border-zinc-700 pb-2">
                ...
                <span className="text-lg font-bold text-zinc-100">{Number(selectedTransaction.amount)}</span>
              </div>
              {selectedTransaction.fees > 0 ? (
                <div className="flex justify-between items-center border-b border-zinc-700 pb-2">
                  <span className="text-zinc-400">Комиссия:</span>
                  <span className="text-zinc-300">{Number(selectedTransaction.fees)}</span>
                </div>
              ) : null}
              <div className="flex justify-between items-center border-b border-zinc-700 pb-2">
                <span className="text-zinc-400">Зачислено:</span>
                <span className="text-lg font-bold text-green-500">{Number(selectedTransaction.total)}</span>
              </div>
              ...
              <div className="flex justify-between items-center border-b border-zinc-700 pb-2">
                ...
                <span className="text-zinc-300">
                  {selectedTransaction.sender === -1 ? strings.system : selectedTransaction.sender}
                  {selectedTransaction.sender_name ? ` (${selectedTransaction.sender_name})` : ''}
                </span>
              </div>
              <div className="flex justify-between items-center border-b border-zinc-700 pb-2">
                <span className="text-zinc-400">Получатель:</span>
                <span className="text-zinc-300">
                  {selectedTransaction.receiver === -1 ? strings.system : selectedTransaction.receiver}
                  {selectedTransaction.receiver_name ? ` (${selectedTransaction.receiver_name})` : ''}
                </span>
              </div>
