...
export default function AccountContent({ accountId }: AccountContentProps) {
  ...
  const getPartyLabel = (accountNumber: number, resolvedName?: string) => {
    if (accountNumber === -1) return strings.system;
    return resolvedName || String(accountNumber);
  };
  ...
  const getTransactionIcon = (trans: WalletTransaction) => {
    if (trans.status === 0) {
      return (
        <div className="border border-zinc-600/30 shrink-0 flex justify-center items-center bg-yellow-500/25 h-10 w-10 lg:h-12 lg:w-12 duration-300 rounded-full">
          <svg className="fill-yellow-500 h-6 w-6 lg:w-8 lg:h-8 inline animate-pulse" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
    ...
    // Successful transaction icons depending on direction
    if (trans.sender === accountId && trans.receiver === accountId) {
      return (
  ...
  const getTransactionTextClass = (trans: WalletTransaction) => {
    ...
    if (trans.sender === accountId && trans.receiver === accountId) return 'text-zinc-500';
    if (trans.sender === accountId) return 'text-red-500';
    return 'text-green-500';
  };

  const getTransactionPrefix = (trans: WalletTransaction) => {
    if (trans.status === 2) return '';
    if (trans.sender === accountId && trans.receiver === accountId) return '';
    if (trans.sender === accountId) return '- ';
    return '+ ';
  };
            ...
            <div className="bg-zinc-800/60 rounded-3xl p-4 w-full text-sm space-y-2.5 border border-zinc-600/30">
              ...
              <div className="flex justify-between items-center border-b border-zinc-700 pb-2">
                ...
                <span className="font-semibold text-zinc-200">
                  {selectedTransaction.is_internal
                    ? 'Внутренний перевод'
                    : (selectedTransaction.direction === 'out' ? 'Исходящий перевод' : 'Входящий перевод')}
                </span>
              </div>
              <div className="flex justify-between items-center border-b border-zinc-700 pb-2">
              ...
              <div className="flex justify-between items-center border-b border-zinc-700 pb-2">
                ...
                <span className="text-zinc-300">
                  ...
                  {!selectedTransaction.is_internal && selectedTransaction.other_party_name && ` (${selectedTransaction.other_party_name})`}
                </span>
              </div>
